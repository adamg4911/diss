...
def calculate_pck(pred, gt, alpha=0.2):
    ...
    for i in range(pred.shape[0]):
        x = [j for j in pred[i][::3]]
        y = [j for j in pred[i][1::3]]
        x2 = [j for j in gt[i][::3]]
        y2 = [j for j in gt[i][1::3]]
        pred2.append([x,y])
        gt2.append([x2,y2])
    pred = np.array(pred2)
    gt = np.array(gt2)
...
def load_config(path="Models/testing/sign_data/text_vocab.txt_1_8_(512, 1024)/config.yaml"):
    """
...
def get_latest_checkpoint(ckpt_dir, post_fix="_best" ) -> Optional[str]:
    """
