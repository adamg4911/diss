...
class TrainManager:
    ...
    def __init__(self, model: Model, config: dict, test=False) -> None:
        ...
        model_continue = True #train_config.get("continue", True)
        ...
        self.model_dir = make_model_dir(train_config["model_dir"], overwrite=False, model_continue=model_continue)
        # Build logger
    ...
    def train_and_validate(self, train_data: Dataset, valid_data: Dataset) \
        ...
        for epoch_no in range(self.epochs):
            ...
            for batch in iter(train_iter):
                ...
                if self.steps % self.validation_freq == 0 and update:
                    ...
                    self._add_report(valid_pck=np.mean(pck),
        ...
        self.logger.info('Best validation result at step %8d: %6.2f %s PCK %8.4f.',
    ...
    def _add_report(self, valid_pck, valid_score: float, valid_loss: float, eval_metric: str,
...
def test(cfg_file,
    ...
    for data_set_name, data_set in data_to_predict.items():
        ...
        f.close()
        print(score, loss, np.mean(all_dtw_scores), np.mean(pck))
